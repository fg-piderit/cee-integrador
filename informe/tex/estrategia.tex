...
(con $\beta = 2$) obtenido en el conjunto de testeo. Sin embargo, también se
calculó la \textit{accuracy}, la precisión, el \textit{recall} y el área bajo la curva
\textit{precision-recall} para poder interpretar mejor los resultados. La
selección del puntaje $F_2$ como métrica principal se basó en la consideración
de las consecuencias de de los falsos positivos y los falsos negativos en la
identificación de tumores malignos. Durante todo el ejercicio se consideró como
clase positiva a los tumores malignos.
...
problema en cuestión. Un falso negativo, en este escenario\footnote{Dado no
estoy familiarizado con el tema y tampoco es el núcleo del trabajo práctico,
para el razonamiento detrás de la elección de la métrica me basé en
\url{https://en.wikipedia.org/wiki/Breast_cancer}}, es el peor de los
resultados ya que implica que un tumor maligno no fue detectado, lo cual puede
tener consecuencias graves sobre la salud de quien sea que lo tiene; si no se
actúa a tiempo, el tumor puede hacer metástasis y alcanzar otras partes del
cuerpo, poniendo en riesgo la vida del paciente. Un falso positivo, si bien no
alcanza el mismo nivel de gravedad, sigue sin ser un buen resultado; significa
que se identificó como maligno un tumor benigno, lo cual puede llevar a que la
persona sufra tratamientos innecesarios. Como estamos lidiando con tumores
mamarios, esto puede significar cirugías, radioterapia o quimioterapia,
...
orden en la gravedad de los errores. Finalmente se optó por usar un valor de
$\beta = 2$, que es relativamente arbitrario
\footnote{Un antecedente del campo de la bioinformática que utiliza un valor de $\beta = 2$ es \cite{sanchez-herrero_etal_2024_forecasting}. Sin embargo, otros antecedentes indican que lo más común es usar $\beta = 1$, y que, de todas formas, el puntaje $F_\beta$ es inadecuado en casos clínicos \parencite{assel_vickers_2025_score}} pero mayor a 1.
