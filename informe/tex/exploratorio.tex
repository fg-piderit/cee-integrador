...
Los datos provienen de la \textit{Diagnostic Wisconsin Breast Cancer Database}
\parencite{williamwolberg_1993_breast}. Cuentan con 569 observaciones y 31
variables, de las cuales una, \verb|target|, es la variable de respuesta que
nos indica si el tumor al que corresponde cada observación es maligno
(\verb|M|) o benigno (\verb|B|), y las demás son variables numéricas
predictoras que resumen la información obtenida de las células de dichos
tumores. Como se mencionó en la introducción, estas variables corresponden a
tres medidas resumen (la media, el desvío estándar y el peor caso) de diez
características celulares listadas a continuación:
...
variables. En el Cuadro \ref{tab:resumen}, presentado en el Anexo
\ref{sec:anexo2} dada su extensión, se muestran algunas medidas resumen de las
variables predictoras, agrupadas según la medida resumen (valga la redundancia)
a la que correspondan. Las variables predictoras tienen unidades sumamente
...
Las Figuras \ref{fig:area} y \ref{fig:smooth} ofrecen visualizaciones de las
distribuciones de las variables asociadas a \verb|area| y \verb|smoothness|. Las
Figuras \ref{fig:radius}-\ref{fig:fractal}, presentadas en el Anexo
\ref{sec:anexo2}, muestran lo mismo para las variables asociadas a las demás
características de las células. La fila superior de cada una muestra
histogramas de densidad según el valor de \verb|target| para cada medida
resumen y la fila inferior muestra un diagrama de dispersión por cada
combinación de dos de esas medidas con los puntos coloreados según los valores
de \verb|target|.
...
el perímetro. Es más débil entre la media y el desvío, yendo de 0.3505 para la
